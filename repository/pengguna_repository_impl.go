...
func (repo *UserRepositoryImpl) SaveUser(ctx context.Context, tx *sql.Tx, user domain.User) error {
	SQL := "INSERT INTO `table_user`(`id_user`, `nama_user`, `no_handphone`, `alamat_asal`, `alamat_sekolah`, `email`, `password_user`, `nama_ortu`, `no_hp_ortu`, `created_at`, `updated_at`) VALUES ('?','?','?','?','?','?','?','?','?','?')"
	id := ksuid.New().String()
	_, err := tx.ExecContext(ctx, SQL, id, user.IdUser, user.NamaUser, user.NoHandphoneUser, user.AlamatUser, user.AlamatSekolah, user.EmailUser, user.PasswordUser, user.NamaOrtu, user.NoHandphoneOrtu, time.Now(), time.Now())
	if err != nil {
		return err
	}
	return nil
...
func (repo *UserRepositoryImpl) UpdateProfileUser(ctx context.Context, tx *sql.Tx, id string, user domain.User) error {
	SQL := "UPDATE `table_user` SET `nama_user`='?',`no_handphone`='?',`alamat_asal`='?',`alamat_sekolah`='?',`email`='?',`nama_ortu`='?',`no_hp_ortu`='?',`updated_at`='?' WHERE id_user = ?"
...
func (repo *UserRepositoryImpl) UpdatePasswordUser(ctx context.Context, tx *sql.Tx, email string, pasword string) error {
	SQL := "UPDATE user SET password_user = ? WHERE email = ?"
