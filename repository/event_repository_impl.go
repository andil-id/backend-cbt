...
func (r *EventRepositoryImpl) SaveEvent(ctx context.Context, tx *sql.Tx, event domain.Events) (string, error) {
	id := ksuid.New().String()
	SQL := "INSERT INTO events (id, title, description, banner, start_at, end_at, created_at, updated_at) VALUES (?,?,?,?,?,?,?,?)"
	_, err := tx.ExecContext(ctx, SQL, id, event.Title, event.Description, event.Banner, event.StartAt, event.EndAt, time.Now(), time.Now())
	if err != nil {
		panic(err)
	}
	if err != nil {
...
func (r *EventRepositoryImpl) GetAllEvents(ctx context.Context, db *sql.DB) ([]domain.Events, error) {
	...
	for rows.Next() {
		...
		err := rows.Scan(&event.Id, &event.Title, &event.Description, &event.Banner, &event.StartAt, &event.EndAt, &event.CreatedAt, &event.UpdatedAt)
		if err != nil {
			panic(err)
		}
